...
df_sales = pd.read_excel(r'C:\Users\SSAFY\Desktop\E205\data\final\final_with_dong\simul_final_with_dong_2021.xlsx')
df_sales
...
df['분기당_매출_금액'] = df['분기당_매출_금액'].fillna(df['분기당_매출_금액'].mean())
df['분기당_매출_금액'] = df['분기당_매출_금액'].replace(np.inf, df['분기당_매출_금액'].mean())
df['분기당_매출_건수'] = df['분기당_매출_건수'].replace(0, df['분기당_매출_건수'].mean())

df['분기당_매출_건수'] = df['분기당_매출_건수'].fillna(df['분기당_매출_건수'].mean())
df['분기당_매출_건수'] = df['분기당_매출_건수'].replace(np.inf, df['분기당_매출_건수'].mean())
df['분기당_매출_건수'] = df['분기당_매출_건수'].replace(0, df['분기당_매출_건수'].mean())

df['점포수'] = df['점포수'].fillna(df['점포수'].mean())
df['점포수'] = df['점포수'].replace(np.inf, df['점포수'].mean())
df['점포수'] = df['점포수'].replace(0, df['점포수'].mean())

df['유사_업종_점포_수'] = df['유사_업종_점포_수'].fillna(df['유사_업종_점포_수'].mean())
df['유사_업종_점포_수'] = df['유사_업종_점포_수'].replace(np.inf, df['유사_업종_점포_수'].mean())
df['유사_업종_점포_수'] = df['유사_업종_점포_수'].replace(0, df['유사_업종_점포_수'].mean())

df['개업_점포_수'] = df['개업_점포_수'].fillna(df['개업_점포_수'].mean())
df['개업_점포_수'] = df['개업_점포_수'].replace(np.inf, df['개업_점포_수'].mean())
df['개업_점포_수'] = df['개업_점포_수'].replace(0, df['개업_점포_수'].mean())

df['폐업_점포_수'] = df['폐업_점포_수'].fillna(df['폐업_점포_수'].mean())
df['폐업_점포_수'] = df['폐업_점포_수'].replace(np.inf, df['폐업_점포_수'].mean())
df['폐업_점포_수'] = df['폐업_점포_수'].replace(0, df['폐업_점포_수'].mean())

df['프랜차이즈_점포_수'] = df['프랜차이즈_점포_수'].fillna(df['프랜차이즈_점포_수'].mean())
df['프랜차이즈_점포_수'] = df['프랜차이즈_점포_수'].replace(np.inf, df['프랜차이즈_점포_수'].mean())
df['프랜차이즈_점포_수'] = df['프랜차이즈_점포_수'].replace(0, df['프랜차이즈_점포_수'].mean())

df['총_생활인구_수'] = df['총_생활인구_수'].fillna(df['총_생활인구_수'].mean())
df['총_생활인구_수'] = df['총_생활인구_수'].replace(np.inf, df['총_생활인구_수'].mean())
df['총_생활인구_수'] = df['총_생활인구_수'].replace(0, df['총_생활인구_수'].mean())

df['남성_생활인구_수'] = df['남성_생활인구_수'].fillna(df['남성_생활인구_수'].mean())
df['남성_생활인구_수'] = df['남성_생활인구_수'].replace(np.inf, df['남성_생활인구_수'].mean())
df['남성_생활인구_수'] = df['남성_생활인구_수'].replace(0, df['남성_생활인구_수'].mean())

df['여성_생활인구_수'] = df['여성_생활인구_수'].fillna(df['여성_생활인구_수'].mean())
df['여성_생활인구_수'] = df['여성_생활인구_수'].replace(np.inf, df['여성_생활인구_수'].mean())
df['여성_생활인구_수'] = df['여성_생활인구_수'].replace(0, df['여성_생활인구_수'].mean())

df['월_평균_소득_금액'] = df['월_평균_소득_금액'].fillna(df['월_평균_소득_금액'].mean())
df['월_평균_소득_금액'] = df['월_평균_소득_금액'].replace(np.inf, df['월_평균_소득_금액'].mean())
df['월_평균_소득_금액'] = df['월_평균_소득_금액'].replace(0, df['월_평균_소득_금액'].mean())

df['소득_구간_코드'] = df['소득_구간_코드'].fillna(df['소득_구간_코드'].mean())
df['소득_구간_코드'] = df['소득_구간_코드'].replace(np.inf, df['소득_구간_코드'].mean())
df['소득_구간_코드'] = df['소득_구간_코드'].replace(0, df['소득_구간_코드'].mean())

df['지출_총금액'] = df['지출_총금액'].fillna(df['지출_총금액'].mean())
df['지출_총금액'] = df['지출_총금액'].replace(np.inf, df['지출_총금액'].mean())
df['지출_총금액'] = df['지출_총금액'].replace(0, df['지출_총금액'].mean())

df['총 상주인구 수'] = df['총 상주인구 수'].fillna(df['총 상주인구 수'].mean())
df['총 상주인구 수'] = df['총 상주인구 수'].replace(np.inf, df['총 상주인구 수'].mean())
df['총 상주인구 수'] = df['총 상주인구 수'].replace(0, df['총 상주인구 수'].mean())

df['총_직장_인구_수'] = df['총_직장_인구_수'].fillna(df['총_직장_인구_수'].mean())
df['총_직장_인구_수'] = df['총_직장_인구_수'].replace(np.inf, df['총_직장_인구_수'].mean())
df['총_직장_인구_수'] = df['총_직장_인구_수'].replace(0, df['총_직장_인구_수'].mean())

df['남성_직장_인구_수'] = df['남성_직장_인구_수'].fillna(df['남성_직장_인구_수'].mean())
df['남성_직장_인구_수'] = df['남성_직장_인구_수'].replace(np.inf, df['남성_직장_인구_수'].mean())
df['남성_직장_인구_수'] = df['남성_직장_인구_수'].replace(0, df['남성_직장_인구_수'].mean())

df['여성_직장_인구_수'] = df['여성_직장_인구_수'].fillna(df['여성_직장_인구_수'].mean())
df['여성_직장_인구_수'] = df['여성_직장_인구_수'].replace(np.inf, df['여성_직장_인구_수'].mean())
df['여성_직장_인구_수'] = df['여성_직장_인구_수'].replace(0, df['여성_직장_인구_수'].mean())

df['아파트_평균_시가'] = df['아파트_평균_시가'].fillna(df['아파트_평균_시가'].mean())
df['아파트_평균_시가'] = df['아파트_평균_시가'].replace(np.inf, df['아파트_평균_시가'].mean())
df['아파트_평균_시가'] = df['아파트_평균_시가'].replace(0, df['아파트_평균_시가'].mean())

df['집객시설_수'] = df['집객시설_수'].fillna(df['집객시설_수'].mean())
df['집객시설_수'] = df['집객시설_수'].replace(np.inf, df['집객시설_수'].mean())
df['집객시설_수'] = df['집객시설_수'].replace(0, df['집객시설_수'].mean())
...
df.drop('분기당_매출_금액', axis=1, inplace=True)

df.to_excel("real_final_2021.xlsx", index=False)
